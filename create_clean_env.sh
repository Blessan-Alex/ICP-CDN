...
# Create a completely clean .env file with the exact JWT token
cat > .env << 'EOF'
...
echo "📏 JWT token length: $(grep 'VITE_PINATA_JWT' .env | cut -d'=' -f2 | wc -c)"
echo "🔑 JWT token (first 50 chars): $(grep 'VITE_PINATA_JWT' .env | cut -d'=' -f2 | cut -c1-50)..."
echo "🔑 JWT token (last 50 chars): ...$(grep 'VITE_PINATA_JWT' .env | cut -d'=' -f2 | rev | cut -c1-50 | rev)"
